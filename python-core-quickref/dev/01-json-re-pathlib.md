...
运行：`python3 01_json_re_pathlib.py`（在 `dev/scripts` 目录）

偏**本地脚本、小工具、读配置**；与力扣 IDE 无直接关系，但日常开发极常用。  
下文各「输入代码 / 输出结果」与脚本中的赋值及 `print` **一一对应**；临时路径和当前目录等动态片段可用 `<TMP_PATH>`、`<CWD>` 理解；改脚本时请同步更新本文（见 [../README.md](../README.md) 维护约定）。
